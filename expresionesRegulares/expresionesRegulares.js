...
function esValidaURL(url){
    let expresion = /^((http(s)?):\/\/)?(www\.)[a-zA-Z0-9]{2,}\.+(com|es)\?+([a-zA-Z0-9]+\=+[a-zA-Z0-9]+([&])){1,}([a-zA-Z0-9]+\=+[a-zA-Z0-9])$/g;
    let resultado = false;
...
function esValidaPassword(password){
    let expresion = /^(?=.*[0-9]+.*){4,}(?=.*[\!|\?|\-|\$|\_])(?=.*[a-zA-Z]+.*){4,}(?=.*[A-Z])([a-zA-Z\d\!|\?|\-|\$|\_]{9,})$/g;
    let resultado = false;
...
function eliminaCaracteresRaros(texto){
     let resultado = texto.replace(/[\W_]+/g, ' ');
     return resultado;
}