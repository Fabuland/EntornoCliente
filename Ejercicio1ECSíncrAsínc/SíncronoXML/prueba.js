function tratarResultadoXML(textoXMLRespuesta) {
    ...
    let rol = datos.documentElement.getElementsByTagName("rol");

    // En la variable salida compondremos el código HTML de la tabla a imprimir.
    let salida = "<table border='1'><tr><th>titulo</th><th>genero</th><th>editorial</th><th>creador</th></tr>";

    // Hacemos un bucle para recorrer todos los elementos CD.
    for (let i = 0; i < rol.length; i++) {
        salida += "<tr>";

        let titulo = rol[i].getElementsByTagName("titulo");
        salida += "<td>" + titulo[0].firstChild.nodeValue + "</td>";

        let genero = rol[i].getElementsByTagName("genero");
        salida += "<td>" + genero[0].firstChild.nodeValue + "</td>";

        let editorial = rol[i].getElementsByTagName("editorial");
        salida += "<td>" + editorial[0].firstChild.nodeValue + "</td>";


        let creador = rol[i].getElementsByTagName("creador");
        salida += "<td>" + creador[0].firstChild.nodeValue + "</td>";
