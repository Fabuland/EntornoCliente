...
function sumarVidas(especie){
    let resultado = listadoAnimales.filter(x => (x.especie === especie)).filter(x => x.nombre.includes(" ")).map(x => x.vida).reduce((sum, x) => sum + x);
...
function dameAnimales(especie,raza){
    ...
    let resultado = listadoAnimales.filter
        (x => x.especie === especie).filter(x => (x.raza === raza)).sort().reverse();
    return console.log(resultado);

}

setTimeout(function(){dameAnimales("Perro","salchicha");}, 10000);
